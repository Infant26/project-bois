...
export default function BookingForm() {
  ...
  async function submitPayAtProperty(event) {
    ...
    try {
      ...
      setMessage(`Booking confirmed. Reference: ${booking.id}`);
    } catch (error) {
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          setMessage(`Payment successful and booking confirmed. Reference: ${booking.id}`);
          setLoading(false);
        },
        modal: {
          ondismiss: function () {
  ...
  return (
    <form className="booking-card" onSubmit={enableRazorpay ? submitWithRazorpay : submitPayAtProperty}>
      ...
      <button disabled={loading || !form.room_id}>{loading ? 'Processing...' : enableRazorpay ? 'Pay & Reserve' : 'Reserve Now'}</button>
